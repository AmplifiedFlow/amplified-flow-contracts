...
// will be transferred to a governance smart contract once amfs is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract AmplifiedFlowFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of amfss
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. amfss to distribute per block.
        uint256 duration;
        uint256 startBlock;
        uint256 bonusEndBlock;
        uint256 lastRewardBlock;  // Last block number that amfss distribution occurs.
        uint256 accTokenPerShare;   // Accumulated amfss per share, times 1e12. See below.
        uint256 accReflectionPerShare;   // Accumulated amfss per share, times 1e12. See below.
    ...
    // amfs tokens created per block.
    uint256 public rewardPerBlock;
    // Bonus muliplier for early amfs makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    address public treasuryWallet = 0xc01c104260DA7258C0Ef333a1612335882528647;
    ...
    // The block number when amfs mining starts.
    ...
    event SetTreasuryWallet(address indexed user, address newAddress);
    event SetPerformanceFee(uint256 fee);
    event UpdateEmissionRate(address indexed user, uint256 rewardPerBlock);

    constructor(IERC20 _amfs, address _reflectionToken, uint256 _rewardPerBlock, bool _hasDividend) {
        amfs = _amfs;
    ...
    // Update the given pool's amfs allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending amfs on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 amfsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(amfsReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.bonusEndBlock);
        uint256 amfsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(amfsReward.mul(1e12).div(lpSupply));
    ...
    function compoundReward(uint256 _pid) external payable nonReentrant {
        ...
        if(address(amfs) != address(pool.lpToken)) {
            uint256 tokenAmt = pending / 2;
            uint256 tokenAmt0 = tokenAmt;
            address token0 = address(amfs);
    ...
    function _transferPerformanceFee() internal {
        ...
        payable(treasuryWallet).transfer(performanceFee);
    ...
    function _calculateTotalStaked(uint256 _pid, IERC20 _lpToken, uint256 _amount, bool _deposit) internal {
        if(_deposit) {
            ...
            if(address(_lpToken) == address(amfs)) {
    ...
    function availableRewardTokens() public view returns (uint256) {
        if(address(amfs) == reflectionToken) return totalEarned;

        uint256 _amount = amfs.balanceOf(address(this));
    ...
    // Safe amfs transfer function, just in case if rounding error causes pool to not have enough amfss.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 amfsBal = amfs.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > amfsBal) {
            transferSuccess = amfs.transfer(_to, amfsBal);
        } else {
            transferSuccess = amfs.transfer(_to, _amount);
        }
    ...
    function setPerformanceFee(uint256 _fee) external {
        require(msg.sender == treasuryWallet, "setPerformanceFee: FORBIDDEN");

        performanceFee = _fee;
        emit SetPerformanceFee(_fee);
    }
    
    function SetTreasuryWallet(address _addr) external {
        require(msg.sender == treasuryWallet, "SetTreasuryWallet: FORBIDDEN");
        treasuryWallet = _addr;
        emit SetTreasuryWallet(msg.sender, _addr);
    }

    //amfs has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    ...
    function depositRewards(uint _amount) external nonReentrant {
        ...
        uint256 beforeAmt = amfs.balanceOf(address(this));
        amfs.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 afterAmt = amfs.balanceOf(address(this));
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(_token != address(amfs) && _token != reflectionToken, "cannot recover reward token or reflection token");
